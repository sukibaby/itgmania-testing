...
To install the Xcode Command Line Tools, you can open a Terminal and type `xcode-select --install`. This will begin the installation for the Xcode Command Line Tools.
...
3. **Run the build commands** (replace arm64 with x64_64 if necessary):
    ```bash
...
You can then run `git submodule status` to be sure all your submodules are present. If they are, you can then run the following commands to build (replace arm64 with x64_64 if necessary):

```bash
cmake -B Build -DSM_USE_VCPKG=OFF -DCMAKE_TOOLCHAIN_FILE=
cmake --build Build/release-arm64 -j $(sysctl -n hw.logicalcpu)
```
