...
static LocalizedString FAILED_SDL_INIT(
    "LowLevelWindow_SDL", "Failed to initialize SDL");
...
std::string LowLevelWindow_SDL::TryVideoMode(
  ...
  if (m_glContext == nullptr || p.bpp != CurrentParams.bpp ||
    ...
    if (bFirstRun) {
      GLenum err = glewInit();
      if (err != GLEW_OK) {
        return ssprintf("glewInit failed: %s", glewGetErrostd::string(err));
      }
    }
    ...
    if (SDL_GetCurrentDisplayMode(displayIndex, &mode) == 0 &&
      ...
      LOG->Info(
          "SDL Display refresh rate detected: %d Hz (displayIndex=%d)",
    ...
    if (SDL_GetCurrentDisplayMode(displayIndex, &mode) == 0 &&
      ...
      refreshRate = mode.refresh_rate;
      LOG->Info(
          "SDL Display refresh rate re-detected: %d Hz (displayIndex=%d)",
          mode.refresh_rate, displayIndex);
    } else {
      if (refreshRate <= 0) {
        refreshRate = 60;
