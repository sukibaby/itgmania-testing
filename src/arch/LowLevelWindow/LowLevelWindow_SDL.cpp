...
static LocalizedString FAILED_SDL_INIT("LowLevelWindow_SDL",
                                       "Failed to initialize SDL");

LowLevelWindow_SDL::LowLevelWindow_SDL()
    : m_window(nullptr), m_glContext(nullptr), m_bWasWindowed(true) {
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    RageException::Throw("%s: %s", FAILED_SDL_INIT.GetValue().c_str(),
                         SDL_GetError());
  }
...
RString LowLevelWindow_SDL::TryVideoMode(const VideoModeParams& p,
                                         bool& bNewDeviceOut) {
  ...
  if (m_glContext == nullptr || p.bpp != CurrentParams.bpp ||
    ...
    m_window = SDL_CreateWindow(p.sWindowTitle.c_str(), SDL_WINDOWPOS_UNDEFINED,
                                SDL_WINDOWPOS_UNDEFINED, p.width, p.height,
                                flags);
    ...
    // Apply vsync preference (SDL returns 0 on success, -1 on failure).
    {
      ...
      if (ret != 0) {
        LOG->Warn("SDL_GL_SetSwapInterval(%d) failed: %s", swapInterval,
                  SDL_GetError());
      }
    }
    ...
    if (SDL_GetCurrentDisplayMode(displayIndex, &mode) == 0 &&
      ...
      LOG->Info("SDL Display refresh rate detected: %d Hz (displayIndex=%d)",
                mode.refresh_rate, displayIndex);
    } else {
      if (refreshRate <= 0) refreshRate = 60;
      LOG->Warn(
          "SDL could not query display refresh rate (displayIndex=%d, "
          "mode.refresh_rate=%d), using %dHz",
          displayIndex, mode.refresh_rate, refreshRate);
    }

    CurrentParams = ActualVideoModeParams(p, windowWidth, windowHeight,
                                          /*renderOffscreen=*/false);
    CurrentParams.rate = refreshRate;
    m_bWasWindowed = p.windowed;
  } else {
    // Remodeling existing window without context change
    bNewDeviceOut = false;
    ...
    if (SDL_GetCurrentDisplayMode(displayIndex, &mode) == 0 &&
        mode.refresh_rate > 0) {
      refreshRate = mode.refresh_rate;
      LOG->Info("SDL Display refresh rate re-detected: %d Hz (displayIndex=%d)",
                mode.refresh_rate, displayIndex);
    } else {
      if (refreshRate <= 0) refreshRate = 60;
      LOG->Warn(
          "SDL could not re-query display refresh rate (displayIndex=%d, "
          "mode.refresh_rate=%d), using %dHz",
          displayIndex, mode.refresh_rate, refreshRate);
    }
    ...
    // Apply vsync preference when remodeling, too.
    {
      const int swapInterval = p.vsync ? 1 : 0;
      const int ret = SDL_GL_SetSwapInterval(swapInterval);
      if (ret != 0) {
  ...
  LOG->Info("Video Mode: %dx%d %d-bit %s", p.width, p.height, p.bpp,
  ...
  if (!bLoggedParamsOnce) {
    ...
    LOG->Info("SDL ActualVideoModeParams: windowed=%d displayId='%s'",
    ...
    LOG->Info(
        ...
        "trilinear=%d aniso=%d",
        CurrentParams.fDisplayAspectRatio,
        CurrentParams.interlaced ? 1 : 0,
        CurrentParams.bSmoothLines ? 1 : 0,
        CurrentParams.bTrilinearFiltering ? 1 : 0,
        CurrentParams.bAnisotropicFiltering ? 1 : 0);
    LOG->Info("SDL ActualVideoModeParams: renderOffscreen=%d",
              CurrentParams.renderOffscreen ? 1 : 0);
    LOG->Info("SDL Window: flags=0x%08x", static_cast<unsigned int>(windowFlags));
  }
...
void LowLevelWindow_SDL::LogDebugInformation() const {
  if (m_window) {
    ...
    LOG->Info("SDL Window Refresh Rate: %d Hz",
              GetActualVideoModeParams().rate);
  }

  LOG->Info("SDL Version: %d.%d.%d", SDL_MAJOR_VERSION, SDL_MINOR_VERSION,
            SDL_PATCHLEVEL);
...
void LowLevelWindow_SDL::SwapBuffers() {
  LOG->Trace("SwapBuffers: About to call SDL_GL_SwapWindow");
  SDL_GL_SwapWindow(m_window);
  LOG->Trace("SwapBuffers: SDL_GL_SwapWindow returned");

  if (PREFSMAN->m_bDisableScreenSaver) {
    // Reset screensaver timer by moving mouse slightly
  ...
  LOG->Trace("SwapBuffers: Complete");
}
...
void LowLevelWindow_SDL::GetDisplaySpecs(DisplaySpecs& out) const {
  ...
  for (int d = 0; d < numDisplays; ++d) {
    ...
    std::string displayId =
        std::string("SDL-Display-") + std::to_string(d);
    std::string displayName =
        std::string("Display ") + std::to_string(d + 1);
    ...
    if (hasCurrentMode && !displayModes.empty()) {
      ...
      RectI bounds(displayBounds.x, displayBounds.y,
                   displayBounds.x + displayBounds.w,
                   displayBounds.y + displayBounds.h);
      out.insert(
          DisplaySpec(displayId, displayName, displayModes, currentMode,
                      bounds));
...
// Note: Concurrent rendering methods below are not implemented because
// SupportsThreadedRendering() returns false. SDL doesn't provide the necessary
// context management (like separate background contexts in X11) to support
// concurrent rendering and texture loading on a background thread.

void LowLevelWindow_SDL::BeginConcurrentRenderingMainThread() {
  // Not implemented - see SupportsThreadedRendering()
}
